...
# - TOP10: 전일 대비 변동(↑n/↓n/—/NEW) 표시
# - 급상승/급하락: ±10계단 이상, 각 최대 5개
# - 랭크 아웃: 전일 Top70 → 금일 Top100에서 제외, 최대 5개
# - 실패 시 Slack에만 공지하고 Exit 0 (원하면 1로 변경 가능)
...
def is_brand_like(s: str) -> bool:
    ...
    # 푸터/메뉴 문구 차단
    bad = ["브랜드", "더보기", "장바구니", "로그인", "고객센터", "대표전화", "채팅", "사업자", "개인정보",
           "청소년", "법적고지", "인스타그램", "페이스북", "유튜브", "카카오톡", "이용약관",
           "공지", "쿠폰", "혜택", "증정", "무배", "오늘드림", "1+1", "업데이트 확인"]
    if any(t in s for t in bad): return False
    return True
    ...
    try:
        ...
        for sel in ["img[alt]", "[aria-label]"]:
            ...
            if el:
                ...
                if is_brand_like(val): return rk, val
        # 텍스트 라인
...
def try_playwright(max_items=100):
    ...
    try:
        with sync_playwright() as p:
            # 브라우저 다중 폴백: chromium → webkit → firefox
            for which in ("chromium", "webkit", "firefox"):
                browser = None
                try:
                    if which == "chromium":
                        ...
                        device = {"viewport": {"width": 414, "height": 896}, "isMobile": True}
                    ...
                    if out:
                        logging.info("Playwright(%s)에서 %d개 추출", which, len(out))
...
def scrape_top100():
    for fn in (try_internal_api, try_playwright, try_requests_html):
        out = fn()
...
def main():
    ...
    if len(brands) < 20:
        send_slack("❌ *올리브영 모바일 브랜드 랭킹* 수집 실패 (접근 차단/데이터 부족) — 브라우저 설치 또는 차단 해제 후 재시도")
        # 실패해도 워크플로우는 성공 처리 (원하면 1로 변경)
        return 0
